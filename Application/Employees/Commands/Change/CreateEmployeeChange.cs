...
{
    public class CreateEmployeeChange
    {
        ...
        public class Handler(AppDbContext context, IMapper mapper, ISybaseService sybaseService) : IRequestHandler<Command, Result<string>>
        {
            ...
            {
                var employee = await context.Employees.FirstOrDefaultAsync(x => x.Id == request.Change.AM, cancellationToken);

                if (employee == null) return Result<string>.Failure("Employee not found", 404);

                request.Change.AM = employee.Am;

                var change = mapper.Map<Domain.Changes>(request.Change);
                ...
                if (!result) return Result<string>.Failure("Failed to create the employee change", 400);
