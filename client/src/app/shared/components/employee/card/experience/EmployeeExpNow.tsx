...
interface EmployeeExpNowProps {
    experienceList?: Experience[];
}

export default function EmployeeExpNow({ experienceList = [] }: EmployeeExpNowProps) {
    ...
    const calculateCurrent = () => {
        ...
        experienceList.forEach((exp) => {
            const carrierUpper = exp.carrier ? exp.carrier: '';
            const isCurrent = carrierUpper.includes(foreas) || carrierUpper.includes(foreas);

            console.log(`Experience: ${exp.carrier}, Type: ${exp.type}, isCurrent: ${isCurrent}`);
            
                        
            if ((exp.type === 4)) {
                // Υπολογισμός με βάση τις ημερομηνίες αντί των προ-υπολογισμένων values
                if (exp.dateFrom && exp.dateTo) {
                    ...
                    // Fallback: χρήση προ-υπολογισμένων τιμών αν δεν υπάρχουν ημερομηνίες
    ...
    const { years, months, days } = calculateCurrent();
    return (
                ...
                <Table className="border-collapse w-full">
                    <TableBody>
                        <TableRow className="border-none text-center">
                            ...
                            <TableCell className="border-r border-l border-gray-300 p-0 w-1/3">
                                <div className="flex flex-col items-center">
