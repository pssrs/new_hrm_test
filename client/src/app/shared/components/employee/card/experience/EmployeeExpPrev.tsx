...
interface EmployeeExpPrevProps {
    experienceList?: Experience[];
}

export default function EmployeeExpPrev({ experienceList = [] }: EmployeeExpPrevProps) {
    ...
    const calculatePrevious = () => {
        ...
        let publicYears = 0, publicMonths = 0, publicDays = 0;
        ...
        experienceList.forEach((exp) => {
            if ((exp.type === 1)) {
                if (exp.dateFrom && exp.dateTo) {
                    ...
                    publicYears += Number(exp.years) || 0;
                    publicMonths += Number(exp.months) || 0;
                    publicDays += Number(exp.days) || 0;
                }
            } else {
        ...
        const totalYears = publicYears;
        const totalMonths = publicMonths;
        const totalDays = publicDays;

        // Κανονικοποίηση
        let y = totalYears, m = totalMonths, d = totalDays;
        if (d >= 30) {
            m += Math.floor(d / 30);
            d = d % 30;
        }
    ...
    const { years, months, days } = calculatePrevious();
